...
3. **`arrange()`:** Esta função é usada para reordenar linhas com base em uma ou mais variáveis.No R: `dados_ordenados <- arrange(dados, variável)` 

4. **`mutate()`:** Esta função é usada para criar ou modificar colunas dentro do conjunto de dados. No R: `dados_modificados <- mutate(dados, nova_coluna = cálculo)`

5. **`group_by()` e `summarize()`:**  Essas funções são usadas em conjunto para agrupar dados com base em uma variável e, em seguida, resumi-los. No R: `dados_agrupados <- group_by(dados, variável)`e `resumo <- summarize(dados_agrupados, estatística_resumo = função(variável))`
...
**3. Mudando o nome**
Podemos mudar o nome das variáveis do banco da seguinte forma: 
