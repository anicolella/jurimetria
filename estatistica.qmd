...
Vamos testar se a média da taxa de feminicídio nos estados brasileiros é igual a 2 assassinatos por 100 mil mulheres. Conforme anunciado por um portal de notícias.

$$H_0: \mu = 2$$ 
$$H_1:\mu \neq 2 $$

```{r}
t.test(final_fem_22$feminic_tx, mu=2) 
```
Podemos testar também a seguinte hipótese: 

$$H_0: \mu = 2$$ 
$$H_1:\mu < 2 $$
```{r}
t.test(final_fem_22$feminic_tx, mu = 2, alternative = "less")

#Esse aqui é para a direita
t.test(final_fem_22$feminic_tx, mu = 2, alternative = "greater")
```
