...
$$feminctx = \beta_0 + \beta_1 txhomici + u$$

Apesar de parecer "muito complicado" na teoria, na prática o `r` estima uma RLS em segundos.
...
#importando os dados
dados<- read_excel("dados_feminic.xlsx")

##Função para rodar a regressão
modelo <- lm(data = dados, dados$feminic_tx ~ dados$homic_tx)
...
# Ajustando a reta de regessão.
 plot(dados$homic_tx, dados$feminic_tx,
     main = "Gráfico de Dispersão: Taxa de Feminicídio vs.tx de homicidio",
...
Temos alguns resultados bem importantes. Note que, ao dividirmos $SQT$ por $n-1$, teremos a variancia amostral de $y$. Por sua vez, ao dividirmos $SQE$ por $n-1$, termeos a variancia populacional de $\hat{y}$.
...
```{r}
###Estimativa da variancia do erro
...
$$ep(\hat{\beta}_1) = \sqrt{Var(\hat{
\beta_1})} = \frac{\hat{\sigma}^2}{\sqrt{SQT_x}}$$


```{r}

###Vamos calcular SQT de x

# media de x
mediax <- mean(dados$homic_tx)
qtx <- dados$homic_tx - mediax

# soma dos quadrados totais de x
sqtx <- sum(qtx^2)

#calculo do erro padrão de b1
ep_b1 <- sqrt(sigma_2_hat) /sqrt(sqtx)
...
Vamos comparar os valores calculados manualmente com os resultados da função `summary`

```{r}

##resumo das estatisticas do modelo
summary(modelo)

```
Note que, que o erro padrão de $\hat{\beta_1}$ e o $R^2$ são iguais aos que calculamos.
...
| Modelo    | Variável Dependente | Variável Explicativa |  Interpretação de $\beta_1$|
|-------------|----------|----------|-----------|
...
rlm <- lm(data = dados, dados$feminic_tx ~ dados$homic_tx + dados$rendapc)
rlm

##Sumarizando os resultados da rlm
...
```{r}
####Aplicação da RLM com variáveis dummy
...
rlm_dummy <- lm(data=dados, dados$feminic_tx ~ dados$homic_tx + CO + S + N +NE)
